            ...
            <form method="POST" action="{{route('news.update', $news->id)}}" enctype="multipart/form-data">
                ...
                <div id="tab1" class="tab-content">
                    <div class="col-md-12 mb-3">
                        ...
                        <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" id="validationTitle" value="{{$news->title}}">
                        @if($errors->has('title'))
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <textarea name="description" class="form-control @error('description') is-invalid @enderror" id="summernote">{{$news->description}}</textarea>
                    ...
                    <div class="col-md-12 mb-3">
                        <select class="form-select" name="tags[]" multiple aria-label="multiple select example">
                            @foreach ($tags as $key => $tag )
                                <option value="{{ $tag->id }}" @foreach($news->news_tags as $index => $tags_news) @if($tags_news->tag_id == $tag->id) selected @endif  @endforeach>{{ $tag->title }}</option>
                            @endforeach
                        </select>
                        {{--                @if($errors->has('tag'))--}}
                        {{--                    <span class="invalid-feedback" role="alert">--}}
                ...
                <div id="tab2" class="tab-content">
                    <div class="row m-3 p-0" id="images">
                        @foreach($news->media as $index=>$image)
                            <input type="hidden" id="count" value="{{count($news->media)}}">
                            <div class="col-md-4" id="old_image_{{$image->id}}">
                                <div class="form-group">
                                    <img width="100%" height="158px" id="img_{{$index}}" src="{{$image->image}}" alt="...">
                                </div>
                                <div class="form-group">
                                    <label>Add Photo <span class="text-danger">*</span></label>
                                    <input type="file" style="margin-bottom: 10px" class="form-control" name="image-{{$index}}" onchange="document.getElementById( `img_{{$index}}`).src = window.URL.createObjectURL(this.files[0])">
                                    <input type="hidden" name="media_{{$index}}" value="{{$image->id}}">
                                    <a onclick="del({{$image->id}})" class="btn btn-danger text-center" style="margin-bottom: 10px" id="removeOldImage"><i class="fa fa-trash"></i></a>
                                </div>
                            </div>
                        @endforeach
        ...
        @section('scripts')
            <script>
                ...
                $(document).on('click', 'del', function (event) {
                    event.preventDefault();
                    $(this).closest('#old_image').remove();
                });

                function del(position) {
                    // console.log(position);
                    $.ajax({
                        url: "{{route('remove_image_news')}}",
                        type: 'POST',
                        data: {
                            "_token": "{{ csrf_token() }}",
                            position: position,
